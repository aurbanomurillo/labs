...
class Labyrinth:
    ...
    def __get_sorrounding_weights__(self, coord_y:int, coord_x:int) -> list[int]: # Devuelve una lista de enteros correspondientes a los pesos de las casillas contiguas a una en coordenadas definidas 
        ...
        for n in range(len(direcciones)):
            ...
            if not ((n == 0 and coord_y == self.rows - 1) or (n == 1 and coord_y == 0) or (n == 2 and coord_x == self.columns - 1) or (n == 3 and coord_x == 0)):
    ...
    def set_all_weights(self):  # Otorga valores de peso a todas las casillas
        ...
        while count < total and self.grid[self.goal.position[0]][self.goal.position[1]].peso == -1 and not weights == self.__get_lab_weights__():
...
if __name__ == "__main__":
    ...
    lab = Labyrinth(15,45,True) # Crea un laberinto
