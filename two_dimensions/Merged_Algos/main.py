...
class Labyrinth:
    ...
    def set_corner_bricks(self): # Incorpora únicamente dos muros que cierran la esquina inferior izquierda
        ...
        bricks:list[tuple[int, int]] = [(len(self.grid)-2, 0), (len(self.grid)-1, 1)]
    ...
    def __actualizar_bricks__(self): # Actualiza todas las casillas de muros 
        ...
        if self.DEBUG:
            ...
            progress.start()
            task = progress.add_task("[magenta]Actualizando muros...", total = total_bricks)
    ...
    def __actualizar__(self, coord_y:int, coord_x:int): # Actualiza una casilla sabiendo sus coordenadas
        ...
        if not self.goal == None:
            ...
            if (coord_y, coord_x) == self.goal.position:
                ...
                self.grid[coord_y][coord_x].symbol =self.grid[coord_y][coord_x].symbols["goal"]
    ...
    def __actualizar_todos__(self):  # Actualiza todas las casillas según su estado
        ...
        if self.DEBUG:
            ...
            progress:Progress = Progress()
            progress.start()
            task = progress.add_task("[magenta]Actualizando casillas...", total = total_tiles)
    ...
    def __nuevo_peso__(self, coord_y:int, coord_x:int) -> int: # Devuelve el nuevo peso de una casilla a partir del peso de las contiguas
    ...
    def __tiles_player_can_advance__(self) -> list[tuple[int, int]]: # Devuelve las casillas a las que se puede mover self.player
        ...
        for direction in directions:
            ...
            # límites correctos
            if ((0 <= coord_y < self.rows and 0 <= coord_x < self.columns) and
                (self.grid[coord_y][coord_x].peso == -1) and
                (not (coord_y, coord_x) in self.bricks) and
                (not self.grid[coord_y][coord_x].estado == "explored")):
    ...
    def brute_force_weights(self):  # Otorga valores de peso a todas las casillas
        ...
        if self.DEBUG:
            ...
            progress:Progress = Progress()
            progress.start()
            task = progress.add_task("[cyan]Otorgando pesos...", total = total_tiles, completed = count)
        ...
        while count < total_tiles and not self.resuelto and not self.imposible_to_solve:
            ...
            for n in range(len(self.grid)):
                ...
                for m in range(len(self.grid[n])):
                    ...
                    if (self.__nuevo_peso__(n, m) == peso_actual) and (not self.grid[n][m].estado == "brick") and (self.grid[n][m].peso == -1):
        ...
        if self.DEBUG:
            ...
            print(f"Se han otorgado los pesos a {count} casillas del total {(self.rows * self.columns) - len(self.bricks) - 2}.")
    ...
    def __advance__(self) -> bool: # Cambia la posición del jugador a la casilla a la que puede avanzar, y si no puede avanzar a las casillas contiguas, la manda a la última casilla en decision_positions[]
        ...
        if len(possible) > 0:
            ...
            if len(possible) > 1:
                ...
                if type(self.player.position) == list:
                    ...
                    current_pos:tuple[int, int] = self.player.position[0], self.player.position[1]
            ...
            self.grid[new_coords[0]][new_coords[1]].peso = self.__nuevo_peso__(new_coords[0], new_coords[1])
    ...
    def dfs_weights(self):
        ...
        if self.DEBUG:
            ...
            progress:Progress = Progress()
            progress.start()
            task = progress.add_task("[cyan]Explorando...", total = total_tiles, completed = count)
        ...
        if self.DEBUG:
            ...
            print(f"Se han otorgado los pesos a {count} casillas del total {total_tiles}.")
            ...
            for _ in range(self.grid[self.goal.position[0]][self.goal.position[1]].peso + 1):
                ...
                while not self.grid[next_path[0]][next_path[1]].peso == 1 and not end:
                    ...
                    while not found and n < len(directions):
                        ...
                        if ((0 <= coord_y < self.rows) and
                            (0 <= coord_x < self.columns) and
                            (self.grid[coord_y][coord_x].peso == self.grid[next_path[0]][next_path[1]].peso - 1)):
...
class Casilla:
    ...
    def set_estado_a(self, new_estado:str): # Define el estado de la casilla, ya sea la casilla de inicio del Start, la casilla de meta, un muro, una casilla vacía, o una casilla que forma parte del camino óptimo
...
class Start:
    ...
    def __init__(self, position_y:int, position_x:int): # Contructor de la clase Start
...
class Meta:
    ...
    def __init__(self, position_y:int, position_x:int): # Constructor de la clase Meta
...
class Player:
    ...
    def __init__(self, position_y:int, position_x:int): # Constructor de la clase Player
...
if __name__ == "__main__":
    ...
    if lab.resuelto:
        ...
        lab.print_lab(["path", "brick", "start", "goal", "explored"]) # Suelta el laberinto por pantalla
    
    else:

        lab.print_lab(["brick", "start", "goal", "explored"]) # Suelta el laberinto por pantalla
